...
<div class="grid_4 alpha single_book">
    ...
    <h3 class="price left-sidebar"><?php print_custom_field('book_price'); ?></h3>
        ...
        <?php // Limited Edition
          $limited = get_custom_field('limited_edition_link:to_link_href');
          if( !empty($limited) ) :
        ?>
        
		<p class="led left-sidebar"><a href="<?php print_custom_field('limited_edition_link:to_link_href','http://yoursite.com/default/page/');?>">There's also a Limited Edition</a></p>
        
        <?php endif; ?>

        <?php // Blurb box
          $featuredblurb = get_custom_field('bookfeaturedblurb');
          if( !empty($featuredblurb) ) :
        ?>        
        <div class="book_blurb_box">
		<p class="book_blurb left-sidebar"><?php echo apply_filters('the_content', get_custom_field('bookfeaturedblurb')); ?></p>
        </div>
        <?php endif; ?>

        <?php // Book Links
          $blinks = get_custom_field('booklinks');
          if( !empty($blinks) ) :
        ?>         
		<div class="book_links"><?php echo apply_filters('the_content', get_custom_field('booklinks')); ?></div>
        <?php endif; ?>
</div>         
...
<div class="grid_8 single_book">         
		...
		<?php // pdf press release
          $pdfpressrelease = get_custom_field('pdf_press_release');
          $image_id = get_custom_field('pdf_press_release:raw');
		...
		<?php echo apply_filters('the_content', get_custom_field('bookcontent')); ?>
